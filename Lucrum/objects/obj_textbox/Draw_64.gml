...
if(cname != "None"){
	...
	draw_text_transformed_color(name_box_text_x, name_box_text_y, cname, 0.5, 0.5, 0, c,c,c,c, 1);
...
#region TYPE 0: NORMAL DIALOGUE
	...
	repeat(charCount){
		...
		switch(effect){
			case 0:	//normal
				draw_text_transformed_color(xx + (cx*charSize), yy+(cy*stringHeight / 2), letter, letterScale, letterScale, 0, col, col, col, col, 1);
				break;
			
			case 1:	//shakey
				draw_text_transformed_color(xx + (cx*charSize)+random_range(-1,1), yy+(cy*stringHeight)+random_range(-1,1), letter, letterScale, letterScale, 0, col, col, col, col, 1);
				break;
			
			case 2:	//wave
				var so = t;
				var shift = sin(so * pi * freq /  room_speed) * amplitude;
				draw_text_transformed_color(xx + (cx*charSize), yy+(cy*stringHeight)+shift, letter, letterScale, letterScale, 0, col, col, col, col, 1);
				break; 
			
			case 3: //colour shift
				var c1 = make_colour_hsv(t+cc, 255, 255);
				var c2 = make_colour_hsv(t+cc+34, 255, 255);
				draw_text_transformed_color(xx + (cx*charSize), yy+(cy*stringHeight), letter, letterScale, letterScale, 0, c1, c1, c2, c2, 1);
				break;
			...
			case 4: //wave AND colour shift
				...
				draw_text_transformed_color(xx + (cx*charSize), yy+(cy*stringHeight)+shift, letter, letterScale, letterScale, 0, c1, c1, c2, c2, 1);
				break; 
			...
			case 5: //spin
				...
				draw_text_transformed_color(xx + (cx*charSize)+mv, yy+(cy*stringHeight)+(stringHeight/2), letter, letterScale, letterScale, shift*20, col, col, col, col, 1);
			...
			case 6: //pulse
				...
				var mv = charSize/2;
				draw_set_valign(fa_middle); draw_set_halign(fa_middle);
				draw_text_transformed_color(xx + (cx*charSize)+mv, yy+(cy*stringHeight)+(stringHeight/2), letter, shift * letterScale, shift * letterScale, 0, col, col, col, col, 1);
				draw_set_valign(fa_top); draw_set_halign(fa_left);
				break;
			...
			case 7:	//flicker
				var so = t + cc;
				var shift = sin(so*pi*freq/room_speed);
				draw_text_transformed_color(xx + (cx*charSize), yy+(cy*stringHeight), letter, letterScale, letterScale, 0, col, col, col, col, shift+random_range(-1,1));
				break; 
		}
	...
	if(charCount >= str_len){
		...
		draw_sprite_ext(finished_effect, finishede_count, finishede_x, finishede_y, finishede_scale, finishede_scale, 0, c_white, 1);
	}
	#endregion
}
#endregion